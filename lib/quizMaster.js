  ...
  this._answering = false;
    for (var l in this._loggedIn) {
      ...
      this._sockets[this._loggedIn[l]].emit('playerRoundSummary', {
        position: pos,
        answer: answer,
        points: this._leaderboard[this._currentQuestion][this._loggedIn[l]].points
      });
    }

    // Done firinf playerRoundSummary
    //setTimeout(function() {
    //  io.sockets.emit('restoreDefault');
    //}, 6000);
    ...
    io.sockets.emit('showAnswer', users);
    setTimeout(function(){
      io.sockets.emit('showLeaderboard', users);
    }, 4000);
...
QuizMaster.prototype.enableAnswering = function () {
  ...
  // Disable in 10 seconds
  var self = this;
  setTimeout(function() { self.disableAnswering(); }, 3000);
};
